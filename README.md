...
- [本地安全回归（推荐）](#本地安全回归推荐)
- [CI 安全工作流](#ci-安全工作流)
- [前端工程化与体验增强（2026Q2）](#前端工程化与体验增强2026q2)
- [文档索引](#文档索引)
...
当前版本同步完成了前端交互、性能与工程化能力升级：

- **交互与可访问性**
  - Workflow 编辑器的模型/Agent/工具选择支持搜索过滤（大列表更易定位）
  - 顶部关键操作文案优化（更偏任务导向，降低新手理解成本）
  - 编辑器左右侧栏补充 `aria-label`，配置区语义标签优化
- **大规模数据性能优化**
  - 知识库文档列表在文档数 > 50 时启用分页（每页 20）
  - Workflow 画布在节点数 > 50 时启用可视区域渲染优化，减少无效渲染开销
- **错误可诊断性**
  - 统一错误提示映射（显存不足、权重损坏、超时、网络、权限），前端直接给出可排查原因
- **测试体系补齐**
  - 新增 Vitest 组件测试（含工作流配置面板关键交互）
  - 新增 Cypress E2E 用例骨架（覆盖工作流编辑核心路径）
- **构建配置可维护性**
  - Vite 配置拆分为 `vite.config.dev.ts` / `vite.config.prod.ts` / `vite.config.shared.ts`
  - `dev/build/preview` 与 `test:unit/test:e2e` 脚本已按新配置接入
- **前端文档与版本记录**
  - 新增 `docs/frontend/COMPONENTS.md`
  - 新增 `docs/frontend/API.md`
  - 新增 `docs/frontend/USAGE.md`
  - 新增 `CHANGELOG.md`（持续记录功能与修复）
