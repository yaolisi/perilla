...
help:
	...
	@echo "  make install-lint-tools"
	@echo "                   - pip install pinned ruff/mypy (backend/requirements/lint-tools.txt; same pins as CI)"
	@echo "  npm run install-lint-tools"
	@echo "                   - Same command as make install-lint-tools (repo root package.json)"
	@echo "  make lint-backend"
	@echo "                   - Ruff E9 + pr_check contract check/format + mypy gate (CI backend-static-analysis)"
	@echo "  make lint"
	...
	@echo "  npm run docker-build-backend / docker-build-frontend / docker-build-all"
	@echo "                   - 同上（仓库根委托 make；须 Docker）"
	...
	@echo "                   - Vitest (frontend/npm run test:unit; CI frontend-build：同 job 内尚有 i18n 基线 + npm audit critical + build)"
	...
	@echo "                   - Vue prod build (vue-tsc+vite; CI frontend-build 末步)"
	...
	@echo "                   - 可选: make docker-build-all 或 npm run docker-build-all（需 Docker；本地镜像冒烟）"
	...
	@echo "                   - 可选: make docker-build-all 或 npm run docker-build-all（需 Docker；镜像冒烟，通常在 prod build 之后）"
