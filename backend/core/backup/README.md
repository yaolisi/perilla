...
- 数据库为**整库**快照/逻辑备份时，通常包含**所有租户**数据；恢复前须评估环境与隔离策略，避免把生产全量数据恢复到错误的实例。
- 网关租户强制、API Key 绑定与审计要求见 **`../../../tutorials/tutorial.md`**（§10）、**`../../../tutorials/tutorial-security-baseline.md`**；部署与目录约束见 **`../../../docs/DEPLOYMENT.md`**。
