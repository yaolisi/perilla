"""
Diffusers-based local image generation runtime.

Current target:
- image_generation + diffusers
- standard Diffusers directory models such as FLUX / FLUX.2 [klein] / SDXL

Implementation note:
- This runtime uses diffusers AutoPipelineForText2Image with trust_remote_code.
- `pipeline_class` in metadata is treated as informative metadata for now; loading
  still goes through AutoPipelineForText2Image to keep the integration generic.
"""
...
class DiffusersImageGenerationRuntime(ImageGenerationRuntime):
    ...
    def model_info(self) -> Dict[str, Any]:
        return {
            ...
            "pipeline_class": self._metadata.get("pipeline_class"),
            "device": self._device,
        }
    ...
    def _load_sync(self) -> bool:
        ...
        logger.info(
            "[DiffusersImageGenerationRuntime] Loading model from %s pipeline=%s pipeline_class=%s",
            self._model_dir,
            self._metadata.get("pipeline"),
            self._metadata.get("pipeline_class"),
        )
