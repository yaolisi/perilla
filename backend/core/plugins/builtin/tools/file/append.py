...
class FileAppendTool(Tool):
    ...
    def description(self) -> str:
        return "Append content to a file. Creates the file if it doesn't exist. Accepts: (1) relative path under workspace, (2) absolute path under allowed roots."
    ...
    async def run(self, input_data: Dict[str, Any], ctx: ToolContext) -> ToolResult:
        try:
            ...
            newline = input_data.get("newline", True)
            ...
            # 追加内容
            if newline and target_abs.exists():
                content = "\n" + content
            bytes_written = target_abs.write_text(
                (target_abs.read_text(encoding=encoding) if target_abs.exists() else "") + content,
                encoding=encoding
            )
            
            logger.info(f"[file.append] Appended {len(content.encode(encoding))} bytes to {target_abs}")
            ...
            logger.exception(f"[file.append] Failed: {e}")
            return ToolResult(success=False, data=None, error=str(e))
