...
def _validate_step(step: Any, idx: int, errors: List[str]) -> None:
    ...
    if not isinstance(step.get("status"), int):
    ...
    if not isinstance(step.get("ts_ms"), int):
...
def _validate_top_level_types(payload: Dict[str, Any], errors: List[str], expected_schema_version: int) -> None:
    checks = [
        ...
        (isinstance(payload.get("generated_at_ms"), int), "generated_at_ms must be int"),
        (isinstance(payload.get("ok"), bool), "ok must be bool"),
        (isinstance(payload.get("failed_step"), str), "failed_step must be string"),
        (isinstance(payload.get("total_steps"), int), "total_steps must be int"),
        (isinstance(payload.get("ok_steps"), int), "ok_steps must be int"),
        (isinstance(payload.get("err_steps"), int), "err_steps must be int"),
    ]
...
def _validate_aggregate_consistency(payload: Dict[str, Any], steps: List[Any], errors: List[str]) -> None:
    ...
    if isinstance(ok_steps, int) and isinstance(err_steps, int) and isinstance(total_steps, int):
...
def _validate_ok_err_step_relation(ok_value: Any, err_steps: Any, errors: List[str]) -> None:
    if not isinstance(ok_value, bool) or not isinstance(err_steps, int):
        return
...
def _validate_ok_total_relation(ok_value: Any, ok_steps: Any, total_steps: Any, errors: List[str]) -> None:
    if not isinstance(ok_value, bool) or not isinstance(ok_steps, int) or not isinstance(total_steps, int):
        return
...
def _validate_time_consistency(payload: Dict[str, Any], steps: List[Any], errors: List[str]) -> None:
    ts_values = [s.get("ts_ms") for s in steps if isinstance(s, dict) and isinstance(s.get("ts_ms"), int)]
    ...
    generated_at_ms = payload.get("generated_at_ms")
    if isinstance(generated_at_ms, int) and ts_values:
