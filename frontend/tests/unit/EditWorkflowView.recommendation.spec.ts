...
vi.mock('@/services/api', () => ({
  ...
  getSystemConfig: vi.fn(async () => ({ settings: {} })),
  ...
  recommendTemplates: () => [
    {
      ...
      score: 8,
      signals: {
        transition_pairs: [{ from: 'weather.query', to: 'flight.booking', weight: 2.1 }],
      },
    },
  ],
  buildTemplateGraph: () => ({
...
describe('EditWorkflowView recommendation focus integration', () => {
  beforeEach(() => {
  ...
  it('sets canvas focus-node-id when clicking recommendation chip', async () => {
    vi.useFakeTimers()
    const wrapper = mount(EditWorkflowView, {
      global: {
        plugins: [makeI18n()],
        stubs: {
          NodeLibrary: true,
