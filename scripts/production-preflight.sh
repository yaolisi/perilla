...
# 顺序对齐 CI backend-static-analysis 单 job：quick-check → test-no-fallback → test-tenant-isolation → helm-chart-check → merge-gate-contract-tests。
...
echo "[production-preflight] 1/5 quick-check (nvmrc + lint-backend)"
bash scripts/quick-check.sh

echo "[production-preflight] 2/5 test-no-fallback + production readiness baseline"
bash scripts/test-no-fallback.sh -q

echo "[production-preflight] 3/5 tenant isolation (pytest -m tenant_isolation)"
make test-tenant-isolation

echo "[production-preflight] 4/5 helm-chart-check"
bash scripts/helm-chart-check.sh

echo "[production-preflight] 5/5 merge-gate contract tests"
